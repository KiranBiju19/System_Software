    ...
    if (count >= csize)
    {
        ...
        for (len = csize, i = 0; len > 0; i++)
        {
            if (hrdisk[i].data == -1)
            {
                ...
                hrdisk[i].link = NULL;      // Initialize the link to NULL
                ...
                {
                    previous->link = &hrdisk[i]; // Link the previous block to current
                }
                printf("%d--",i);
                previous = &hrdisk[i];  // Update previous to the current block
            }
        }
...
void search()
{
    ...
    while (temp != NULL)
    {
        printf("%ld ", temp - hrdisk);  // Print the block index
        temp = temp->link;  // Move to the next linked block
    }
    printf("\n");
}
